...
names(stack) <- c('temperature','precipitation')
...
environment[['2015']] <- list(
  ...
  #output file 
  stack_name = 'session2_2015_stack.tif'
)
...
environment[['2100_ssp1']] <- list(
  ...
  #output file 
  stack_name = 'session2_2100_ssp1_stack.tif'
)
...
levels(landuse_processed) <- data.frame(ID = seq_along(1:8), Class = c('Forests (primary)','Forests (secondary)',
                                                          'Vegetated lands (primary)','Vegetated lands (secondary)',
                                                          'Pastures / Rangelands','Croplands','Urban','Water'))
